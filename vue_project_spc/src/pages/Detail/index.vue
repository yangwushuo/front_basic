    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImageList="getSkuImageList" />
          <!-- 小图列表 -->
          <ImageList :skuImageList="getSkuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      //1发送请求给服务器，进行添加
      //服务器写入数据成功，并没有返回其他得数据，只是返回code=200，代表添加成功
      //因为服务器没有返回其余数据，因此不需要三连环存储
      try {
